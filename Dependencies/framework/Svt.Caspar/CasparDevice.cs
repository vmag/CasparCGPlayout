...
namespace Svt.Caspar
{
	...
	{
		private readonly Svt.Network.ServerConnection server_ = new Svt.Network.ServerConnection();
		private readonly TemplatesCollection templates_ = new TemplatesCollection();
		private List<MediaInfo> mediafiles_ = new List<MediaInfo>();
		private List<string> datafiles_ = new List<string>();
		private readonly List<Channel> channels_ = new List<Channel>();
		private readonly CasparDeviceSettings settings_ = new CasparDeviceSettings();
		...
		{
			get { return (server_ != null) && server_.IsConnected; }
		}
